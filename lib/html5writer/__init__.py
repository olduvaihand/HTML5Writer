# html5writer/__init__.py

"""Simple HyperText Markup Language document tree Writer.

Derived from David Goodger's html4css1 Writer in
:mod:`docutils.writers.html4css1`.  This software is in the public domain.
...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def depart_document(self, node):
        ...
        self.body_prefix.append(self.starttag(node, 'article'))
        self.body_suffix.insert(0, '</article>\n')
    ...
    def depart_footer(self, node):
        start = self.context.pop()
        footer = [self.starttag(node, 'footer'), '<hr class="footer">\n']
    ...
    def visit_legend(self, node):
        self.body.append(self.starttag(node, 'legend'))
    ...
    def visit_literal(self, node):
        ...
        self.body.append(
            self.starttag(node, 'code', '', CLASS='literal'))
        text = node.astext()
    ...
    def visit_table(self, node):
        classes = self.settings.table_style.strip()
        self.body.append(
